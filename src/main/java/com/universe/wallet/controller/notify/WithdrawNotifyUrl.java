...
import com.universe.wallet.DTO.notify.WithdrawNotifyBody;
...
public class WithdrawNotifyUrl {
	...
	public String withdrawNotify(@RequestBody WithdrawNotifyBody withdrawNotifyBody) {
		log.info("/api/notify/withdraw result: {}", JSONObject.toJSONString(withdrawNotifyBody));
		boolean match = WalletRequestUtil.checkSign((JSONObject) JSON.toJSON(withdrawNotifyBody),
				walletApiProperties.getOpenApi().getWithdraw().getAppKey());
		//状态: 1.待审核, 2.待匹配, 3.待付款, 4.付款延時, 5.待放行, 6.放行延時, 7.投訴 8.維權 9.交易成功 10.交易失败
		if(match) {
			// TODO 签名验证成功，回调收到的数据可信赖， 在这里执行客户自己的处理逻辑，比如对withdrawNotifyBody的数据进行存储等
